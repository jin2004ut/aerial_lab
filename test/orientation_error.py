...
def vee_map(skew_matrix: np.ndarray) -> np.ndarray:
    ...
    if skew_matrix.ndim == 2:
        ...
        # Batch case
        return np.stack([skew_matrix[:, 2, 1], skew_matrix[:, 0, 2], skew_matrix[:, 1, 0]], axis=-1)
...
if __name__ == "__main__":
    ...
    print(
        "Rotation error error (mean):"
        f" {np.mean(np.abs(np.array(angles_true) - np.array(angles_rotation_error))):.6f} rad"
    )
