        ...
        with self.ui_window_elements["main_vstack"]:
            with self.ui_window_elements["debug_frame"]:
                with self.ui_window_elements["debug_vstack"]:
                    ...
                    self._create_debug_vis_ui_element("forces", self.env)
...
class BeetleEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self._actions = actions.clone().clamp(-1.0, 1.0)  # TODO: add action limits
    ...
    def _apply_action(self) -> None:
        ...
        # ### Sim-1 : joint reaction torques on rotor joints (joint driver target type = velocity)
        # self._robot.set_joint_effort_target(self._target_rotor_torque, self._rotor_ids[0])

        # ### Sim-2 : external (resultant) torques on base bodies, forces on rotor bodies
        ...
        # ### Sim-3 : direct torques/forces on rotor bodies
        target_thrust = torch.zeros(self.num_envs, self.cfg.rotor_num, 3, device=self.device)
        target_thrust[:, :, 2] = self._target_thrust_force
        target_torque = torch.zeros(self.num_envs, self.cfg.rotor_num, 3, device=self.device)
        target_torque[:, :, 2] = self._target_rotor_torque
        self._robot.set_external_force_and_torque(
            forces=target_thrust, torques=target_torque, body_ids=self._rotor_ids[0]
        )
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # crash = torch.linalg.norm(self._contact_sensor.data.net_forces_w.squeeze(1), dim=-1) > self.cfg.contact_force_threshold
        ...
        # print("Contact forces force_matrix_w: ", torch.linalg.norm(self._contact_sensor.data.force_matrix_w.squeeze(1), dim=-1))
        # print("Contact forces net_forces_w: ", torch.linalg.norm(self._contact_sensor.data.net_forces_w.squeeze(1), dim=-1))
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        ...
        if debug_vis:
            ...
            if not hasattr(self, "force_visualizer"):
                ...
                marker_cfg.markers["arrow_x"].scale = (0.05, 0.05, 0.05)
    ...
    def _debug_vis_callback(self, event):
        ...
        # contact_forces = self._contact_sensor.data.net_forces_w.squeeze(1)
        # forces_pos = self._contact_sensor.data.pos_w.squeeze(1)
        # forces_mag = torch.linalg.norm(contact_forces, dim=-1)
        # scales = torch.ones_like(contact_forces) * 0.1
        # scales[:, 0] = forces_mag * 0.5
        # forces_dir = quat_from_angle_axis(torch.zeros(forces_mag.shape[0]), contact_forces)
        # self.force_visualizer.visualize(
        #     translations=forces_pos,
        #     orientations=forces_dir,
        #     scales=scales
        # )
