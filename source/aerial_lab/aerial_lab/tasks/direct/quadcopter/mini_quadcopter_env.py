...
@configclass
class MiniQuadcopterEnvCfg(DirectRLEnvCfg):
    # env
    sim_dt = 1 / 200.0
    decimation = 4
    play_mode = False
    ...
    max_curricular_steps = 1  # no curriculum
    # - spaces definition
    rotor_num = 4
    gimbal_num = 0
    # 4 gimbals, 4 rotors
    ...
    # servo positions (4)
    # last action (8)
    observation_space = 9 + 6 + 3 + gimbal_num + action_space
    thrust_to_torque_ratio = -0.011  # -0.0165
    ...
    class control:
        body_ang = math.pi / 180.0 * 0.0  # body tilt angle for desired orientation sampling
        clip_observations = 100.0
        clip_actions = 100.0
        # class control:
        # gimbal_action_scale = 0.25
        thrust_action_scale = 2.25
    ...
    thrust_power_reward_scale = -5.0e-4  # -1.0e-4
    # goal_orientation_reward_scale = -0.001
    distance_to_goal_reward_scale = 0.0
    ...
    rotorCfg = {
        ...
        "max_vel": 200.0,  # below unused currently
        "vel_wn": 1.0,
        "vel_zeta": 0.8,
        "foc_wn": 1.0,
        "foc_zeta": 0.8,
    ...
    thrust_sensor: ContactSensorCfg = ContactSensorCfg(
        prim_path="/World/envs/env_.*/Robot/thrust.*",  # Bind to all rotor parent links
        history_length=1,
        update_period=0,
        track_air_time=True,
        track_pose=True,
...
class MiniQuadcopterEnv(DirectRLEnv):
    cfg: MiniQuadcopterEnvCfg

    def __init__(self, cfg: MiniQuadcopterEnvCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)
        try:
            print(f"MiniQuadcopterEnv init: cfg class = {cfg.__class__.__name__}")
            print(f"cfg.robot_cfg.asset_path = {getattr(cfg.robot_cfg.spawn, 'asset_path', None)}")
        except Exception as e:
            print("MiniQuadcopterEnv init: failed to print cfg.robot_cfg:", e)
        # Basic cfgs
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # angular_to_goal_mapped = (1 - torch.tanh(angular_to_goal)) * (0.05 + distance_to_goal_weight)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if self.cfg.evaluate_mode:
            ...
            self._desired_quat_w[env_ids] = sampleUniformQuatwithTilt(torch.tensor(ang_range), len(env_ids)).to(
                self.device
            )
